...
Os eventos públicos são enviados em lotes para `/api/events` e salvos em `funnel_events` sem Instagram, respostas, IP ou identificadores de anúncios. Cada etapa conta no máximo uma vez por sessão de aba (`sessionStorage` + restrição única no banco): atualizar a página na mesma aba não soma outro `page_view`, mas uma nova sessão pode ser contada novamente. O painel chama essa métrica de **sessões**, não visitantes únicos. O funil de aquisição registra `page_view` → `hero_cta_view` (metade do CTA visível) → `hero_cta_clicked` (clique no CTA da landing ou header) → `quiz_visible` (quiz trazido à tela) → `quiz_started` (primeira resposta) → `quiz_question_1_answered` → `quiz_completed`. `click_quiz` permanece para compatibilidade. Os novos estágios não têm dados retroativos; no período inicial, comparar etapas antigas com novas pode produzir percentuais incomparáveis. Eventos posteriores de VSL, oferta e WhatsApp continuam disponíveis. O envio para GA4/Meta e os eventos legados do navegador permanecem. A copy de headline, texto e botão pode ser editada no painel para testar variações sem alterar código; não há divisão automática de tráfego entre variantes. As tabelas são criadas com `CREATE TABLE IF NOT EXISTS`; nenhum dado antigo é apagado.
